...
const { verificarDatos, verificarAdmin}= require('../middlewares/validarUsuario');

const router = Router();
const { agregar, login, obtener, obtenerId} = require('../models/models.usuario.js');
// const { verificarUsuario } = require('../middlewares/middlewares');

router.route('/')
  .post(verificarDatos, async (req, res) => {
    const { usuario, nombreApellido, email, telefono, direccion, contrasena} = req.body;
    const datos = { usuario, nombreApellido, email, telefono, direccion};
    await agregar(usuario, nombreApellido, email, telefono, direccion, contrasena);
